...
from common.variables import (
    DEFAULT_PORT, MAX_CONNECTIONS, ACTION, TIME, USER, ACCOUNT_NAME,
    SENDER, PRESENCE, ERROR, MESSAGE, MESSAGE_TEXT, RESPONSE_400,
...
class ServerSocket(metaclass=ServerMaker):
    port = Port()
...
def main():
    ...
    serv = ServerSocket(listen_address, listen_port)

    # Готовим сокет
    transport = serv.get_socket()
    ...
    while True:
        ...
        for i in messages:
            try:
                serv.process_message(i, names, send_data_lst)
            except Exception:
